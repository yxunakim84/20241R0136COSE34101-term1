...
// 		if (time_quantum > 5 || time_quantum < 1) printf("[ERROR] Time Quantum must be 1 to 5. Please enter time quantum again: ");
// 		else break;
//   }
...
//     }
//     if (IsQueueEmpty(&running_queue)) { 
//       printf("TIME %d ~ %d\t: IDLE\n", time, time + 1); _ROUND_ROBIN.idle_time++;
//     }
//     else {
//       printf("TIME %d ~ %d\t: P[%d] / [%s] \n", time, time + 1, processes[selected]->pid, processes[selected]->entered == TRUE ? "✓" : " ");
//       if(processes[selected]->entered == TRUE) {
...
//     total_burst_time += processes[i]->burst_time;
//   }
//   _ROUND_ROBIN.avg_turnaround_time = (float)total_turnaround_time / process_num;
//   _ROUND_ROBIN.avg_waiting_time = (float)(total_turnaround_time - total_burst_time) / process_num;
...
void ROUND_ROBIN() {
  ...
  while (1) {
    ...
    if (time_quantum > 5 || time_quantum < 1)
      printf("[ERROR] Time Quantum must be 1 to 5. Please enter time quantum again: ");
    else
      break;
  }
